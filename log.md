...
Bug fixes:
1. fixed void* for float and half input argument
2. fixed weight quant as extra input in some kernel functions
3. fixed lib includes 
4. fixed error handling on device side
5. fixed return type of init_checkpoint
6. fixed function name of Memcpy
7. fixed file name in cmakelist
8. fixed kernel name mismatch 
9. deleted repeated function in run.cu
10. fixed size_t incompatible in std::max
11. used template specilization for overloading 

# Changes (until Nov.25)
1. Changed project structure to one file format for compiling simplicity (all necessary files are contained in one .cu file and no extra headers)
2. Fully debugged the code for data type fp16.  
3. Wrote python scripts to generate results.
4. Finished report.

# Changes (until Nov.29)
1. Implemented MHA with fused kernels and wrote benchmarking code.
2. Generated MHA performance result and added to the report.
3. Implemented MHA inference but not fully debugged.

# Changes (until Nov.30)
1. Fixed github .gitignore.
2. Fixed bug in MHA code.
